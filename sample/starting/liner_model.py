...
class LinerModel:
    ...
    def __optimize_step__(self, features, labels, step=0.1):
        ...
        已知：F(X)ij = Wij * Xj + Bi; LOSS(X, Y) = F(X)ij - Yi; LOSS = 1/m∑(F(X)ij - Yi)^2。
        其中m为样本总量，求和公示对所有的特征计算数值和标签数值求方差。对权重Wij求偏导的过程如下：

        展开第一个样本数据的方差：

        LOSS(features[0], labels[0]) = L =
        ...
        在编码实现实际算法的时候，还是使用矩阵结合求和公式实现算法，这样通过numpy执行会高效很多，代码也相对简洁：
            观察Δ的表达式：其中的 Wij × Xj + Bi - Yi 就是运算模型减去标签值，所以可以先进行如下的矩阵计算：
